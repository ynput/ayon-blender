...
from ayon_core.lib import get_ayon_launcher_args, get_launcher_storage_dir
...
def _init_tool_controllers() -> _ToolBackends:
    ...
    if _IPCConnection.ui_backends is not None:
...
def _launch_ui_process(ipc_host: str, ipc_port: int, session_token: str) -> subprocess.Popen:
    env = os.environ.copy()
...
class IPCHandler:
    ...
    @classmethod
    def execute(
        cls, channel: str, method_name: str, params: dict[str, Any] | None
    ) -> bool:
        _ensure_external_ui_process()
        server = _IPCConnection.server
        if not server:
